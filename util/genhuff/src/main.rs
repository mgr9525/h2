...
pub fn main() {
    ...
    println!("pub const ENCODE_CODES: [u32; 256] = [");
    for (_, val) in encode.iter().take(256) {
        println!("    0x{},", val);
    }
    println!("];");

    println!();
    println!("pub const ENCODE_CODE_LENGTHS: [u8; 256] = [");
    for (nbits, _) in encode.iter().take(256) {
        println!("    {},", nbits);
    }
    println!("];");

    println!();
    println!("const _: [(); 1024] = [(); std::mem::size_of::<[u32; 256]>()];");
    println!("const _: [(); 256] = [(); std::mem::size_of::<[u8; 256]>()];");
